...
/**
 * Various functions for measurements in wireless networks. RSSI related
 * computations are based on <a
 * href="http://www.ti.com/lit/an/swra095/swra095.pdf">the formula provided by
 * Texas Instruments</a>, which represented the relationship between RSSI and
 * the estimated 1-D distance. IEEE 802's RSSI are unitless and in the range 0
 * to 255, expressible as a one-byte unsigned integer. RTT is in mili-seconds
 * and related computiation are based on the asumption that speed of light in
 * copper or fibre is 0.6*c (velocity of light), i.e. RTT is 1ms/100km, and in
 * routed networks we can correction factor ~400 m/ms (depending on landmark and
 * target region). Trilateration is <a
 * href="http://en.wikipedia.org/wiki/Trilateration">based on Wikipedia</a>.
 *
 * @author rychly
 */
public final class WirelessMetric {
    ...
    /**
     * Compute a received signal strength at 1 meter distance according to a
     * reference node and its RSSIs and actual distances to nodes A and B.
     *
     ...
     * @throws WirelessMetricException if cannot perform computation with the
     * such parameters
     */
    ...
    /**
     * Compute a propagation constant according to a reference node and its
     * RSSIs and actual distances to nodes A and B.
     *
     * @param rssiToA RSSI value of node A in the reference node
     * @param rssiToB RSSI value of node B in the reference node
     * @param distanceToA distance of node A from the reference node in meters
    ...
    /**
     * Compute a received signal RSSI value from distance of a reference node
     * and a blind node, the received signal strength at 1 meter distance, and
    ...
    /**
     * Compute a distance in meters of a reference node and a blind node from
     * received signal RSSI value and strength at 1 meter distance and
     * propagation constant.
     *
    ...
    /**
     * Compute a correction factor according to a reference node and its RTT and
     * actual distance to another node.
     *
     * @param roundTripTime RTT (round trip time) value in mili-seconds
     * @param distance distance of the another node from the reference node in
     * meters
    ...
    /**
     * Compute a distance in meters of a reference node and a blind node from
     * RTT value and a correction factor.
     *
     * @param roundTripTime RTT (round trip time) value in mili-seconds
     * @param correctionFactor correction factor (for routed networks usually
     * 400 m/ms)
     * @return the distance in meters
     */
    ...
    /**
     * Performs (undeterministic) trilateration in 3D according to locations of
     * nodes A, B, C, and their distances from a blind node.
     *
    ...
    /**
     * Performs (deterministic) trilateration in 3D according to locations of
     * nodes A, B, C, D, and their distances from a blind node.
     *
     * @param nodeA location of node A
     * @param distanceA distance of node A and the blind node
     * @param nodeB location of node B
